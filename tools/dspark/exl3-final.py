...
def apply_exl3_experts(
    ...
    *,
    ...
    # _MIXEDK_GUARD (Fable Q1.1): uniform-K exllamav3 fused path must never
    # run on mixed-K layers (multik patch re-arms _exl3_ptrs via
    # build_exl3_fused_state; exl3_moe would launch with the single layer K).
    have_ptrs = bool(getattr(layer, "_exl3_ptrs", None)) and not bool(
        getattr(layer, "_exl3_mixed_k", False)
    )
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if mixed_k:
            ...
            if (
                ...
                and all(2 <= int(k) <= 8 for k in k_values)
            ):
